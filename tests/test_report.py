...
import hlp
# from init_db import init_db, init_servercmdb_table
...
class TestReport:
    ...
    def test_launch_report_state_change(self, mocker, mock_api_post_return_value):
        ...
        test_report.api.post.assert_called_once_with(
            {'servers': [{'ip': '1.1.1.1'}, {'ip': '2.2.2.2'}], 'title': 'test title'})
    ...
    @pytest.mark.parametrize('all_patches,already_assigned_patches', [pytest.param(test_patches, [], id='all_new'),
                                                                      pytest.param(test_patches, test_patches[:1],
                                                                                   id='one_already_assigned')])
    def test_process_response_patches(self, mocker, patch_mock, all_patches, already_assigned_patches):
        ...
        Report.request.request_servers['ip'].patches = [patch_mock(patch_dict) for patch_dict in
                                                        already_assigned_patches]

        test_report = Report()
        session_mock = mocker.Mock()
        test_report._process_response_patches(session_mock, 'ip', all_patches)

        assert [patch['qid'] for patch in all_patches] == [patch.qid for patch in
    ...
    @pytest.mark.parametrize('all_vulners,already_assigned_vulners', [pytest.param(test_vulners, [], id='all_new'),
                                                                      pytest.param(test_vulners, test_vulners[:1],
                                                                                   id='one_already_assigned')])
    def test_process_response_vulners(self, mocker, vulner_mock, all_vulners, already_assigned_vulners):
        ...
        mocker.patch.object(Report, 'request')
        Report.request.request_servers['ip'].vulners = [vulner_mock(vulner_dict) for vulner_dict in
                                                        already_assigned_vulners]

        test_report = Report()
        session_mock = mocker.Mock()
        test_report._process_response_vulners(session_mock, 'ip', all_vulners)

        assert [vulner['qid'] for vulner in all_vulners] == [vulner.qid for vulner in
